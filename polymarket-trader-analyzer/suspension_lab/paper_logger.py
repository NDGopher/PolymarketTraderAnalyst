...
def run_headless(runtime: LabRuntime, *, duration_seconds: float = 0) -> Path:
    ...
    if runtime.has_known_markets():
        print("Known markets pinned — WS only, no 60s /series+/markets rediscover.", flush=True)
...
def run_paper_logger(
    *,
    ...
    runtime = LabRuntime(
        ...
        max_games=8,
        min_volume=50,
        rediscover_seconds=max(rediscover_seconds, 30.0),  # empty-start only; seated never scans
