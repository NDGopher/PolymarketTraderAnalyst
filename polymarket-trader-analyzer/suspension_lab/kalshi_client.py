...
class KalshiBookFeed:
    """Live L2 via WebSocket. REST is seed-on-connect or explicit --rest-only."""
    ...
    def _emit_note(self, msg: str) -> None:
        if self.on_note:
            self.on_note(msg)
    ...
    def _mark_rate_limit(self, wait: float, *, ticker: str = "") -> None:
        wait = max(wait, DEFAULT_RETRY_AFTER_S)
        self.rate_limit_until = max(self.rate_limit_until, time.monotonic() + wait)
        label = f" {ticker}" if ticker else ""
        msg = f"REST 429{label} - cooling {wait:.0f}s (Retry-After, no tight-loop)"
    ...
    def _fetch_rest_snapshot(self, session: requests.Session, ticker: str) -> bool:
        ...
        try:
            ...
            if resp.status_code == 429:
                ...
                self._mark_rate_limit(wait, ticker=ticker)
                time.sleep(wait)
                return False
            if resp.status_code != 200:
                self._emit_status(f"REST {ticker}: HTTP {resp.status_code}")
                return False
        ...
        while not self._stop.is_set():
            ...
            try:
                ...
                async with websockets.connect(
                    ...
                    # Seed-on-connect only. One ticker at a time; 429 backs off to notes.txt.
